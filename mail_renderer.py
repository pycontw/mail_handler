...
@click.argument('receive_data_json', type=click.Path(exists=True))
@click.option('--output_path', type=click.Path(exists=False), default='mails_to_sent', show_default=True)
def main(template_path, receive_data_json, output_path):
    if not os.path.isdir(output_path):
        logging.info('Create directory "%s"', output_path)
        os.mkdir(output_path)

    with open(receive_data_json, 'r') as input_file:
        receive_data = json.load(input_file)
        common_data = receive_data['common_data']
        unique_data = receive_data['unique_data']
